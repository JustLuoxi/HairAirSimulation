...
const QVector3D AIRGRID_CENTER(0.0f, 0.0f, 0.0f);
...
void OGLManager::handleKeyReleaseEvent(QKeyEvent *event){
    ...
    if(event->key()==Qt::Key_V)
        air->render_flag = !air->render_flag;
    if(event->key()==Qt::Key_H)
        air->hair2air_flag = !air->hair2air_flag;
    if(event->key()==Qt::Key_J)
        air2hair_flag = !air2hair_flag;
}
...
//    core->glBindBufferRange();
    ...
    /************ obj ***********/
    objModel = new Model();
    objModel->init("://res/models/head/Pasha_guard_head_just_hair.obj");
...
//    /************ shader ***********/
    ...
    ResourceManager::loadShader("hair", ":/shaders/res/shaders/hair.vert", ":/shaders/res/shaders/hair.frag");
    ResourceManager::loadShader("air", ":/shaders/res/shaders/air.vert", ":/shaders/res/shaders/air.frag");
    ...
    ResourceManager::getShader("hair").use().setMatrix4f("model", model_);
    ...
    model.setToIdentity();
    model.translate(AIRGRID_CENTER);
    ResourceManager::getShader("air").use().setMatrix4f("model", model);
...
//    /*********  light ************/
    ...
    light->drawLight();
        ...
        // just.air
    ResourceManager::getShader("air").use();
    air->Draw();
...
//    /*********  draw hair ************/
    ResourceManager::getShader("hair").use();
    objModel->m_hair->Draw(this->isOpenLighting);
...
//        camera->processKeyboard(DOWN, dt);
    ...
    // for air interactionw
    QVector3D v;
    if(keys[Qt::Key_W])
    {
        v =  QVector3D(0,100.0f,0);
        air->AddVelocity(RESOLUTION/2,2,RESOLUTION/2,v.x(),v.y(),v.z());
    }
    if(keys[Qt::Key_D])
    {
        v =  QVector3D(100.0f,0,0);
        air->AddVelocity(2,RESOLUTION/2,RESOLUTION/2,v.x(),v.y(),v.z());
    }
}
...
//        air->UpdateRenderdata();
        air->Simulate();
        ResourceManager::getShader("air").use().setMatrix4f("projection", projection);
        ResourceManager::getShader("air").use().setMatrix4f("view", view);
        ResourceManager::getShader("air").use().setMatrix4f("model", model);
        ...
        objModel->m_hair->Update(model);
        ResourceManager::getShader("hair").use().setMatrix4f("projection", projection);
        ResourceManager::getShader("hair").use().setMatrix4f("view", view);
        ResourceManager::getShader("hair").use().setMatrix4f("model", identity_matrix);
        ResourceManager::getShader("hair").use().setVector3f("viewPos", camera->position);
    }
    else //
    {
        ResourceManager::getShader("light").use().setMatrix4f("projection", projection);
        ...
        objModel->m_hair->Update(this->frame_matrixs[current_frame]);
        ResourceManager::getShader("hair").use().setMatrix4f("projection", projection);
        ResourceManager::getShader("hair").use().setMatrix4f("view", view);
        ResourceManager::getShader("hair").use().setMatrix4f("model", identity_matrix);
        ResourceManager::getShader("hair").use().setVector3f("viewPos", camera->position);
...
//                current_frame = 0;
    ...
    //  ResourceManager::getShader("model").use().setMatrix4f("model", model);
    qDebug() << "current FPS: " << 1.0f/deltaTime;
}
...
QMatrix4x4 RotationMatrix4D(Quaternion q)
{
    ...
    QMatrix4x4 result;
    result(0,0)=n00; result(0,1)=n01; result(0,2)=n02; result(0,3)=0.0f;
    result(1,0)=n10; result(1,1)=n11; result(1,2)=n12; result(1,3)=0.0f;
    result(2,0)=n20; result(2,1)=n21; result(2,2)=n22; result(2,3)=0.0f;
...
{
    float Magnitude = std::sqrt(Norm(q));
